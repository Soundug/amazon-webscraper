...
def my_form_post():
    ...
    search_queries.append(processed_text)

    if len(search_queries) >= 1:

        Product_search = search_queries[0]
        ...
        cursor.execute("SELECT * FROM scrapedb.flipkart")
        my_results = cursor.fetchall()

        for result in my_results:
            flipkart_upload.append(result)

        cursor.execute("SElECT * FROM scrapedb.amazon")
        my_results_2 = cursor.fetchall()

        for result in my_results_2:
            amazon_upload.append(result)

        cursor.execute("SELECT * FROM scrapedb.snapdeal")
